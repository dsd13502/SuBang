...
public class SimpleBackActivity extends BaseActivity implements
    ...
    protected void init(Bundle savedInstanceState) {
        ...
        if (mPageValue == -1) {
            mPageValue = getIntent().getIntExtra(BUNDLE_KEY_PAGE, 0);//赋予当前需要调用的 Fragment 代号
        }
    ...
    protected void initFromIntent(int pageValue, Intent data) {
        ...
        SimpleBackPage page = SimpleBackPage.getPageByValue(pageValue); //返回代号对应的 page 的相关信息
        if (page == null) {
            throw new IllegalArgumentException("can not find page by value:"
                    + pageValue);
        }

        setActionBarTitle(page.getTitle()); //设置title

        try {
            Fragment fragment = (Fragment) page.getClz().newInstance(); //获取 page 中的Fragment 类。

            Bundle args = data.getBundleExtra(BUNDLE_KEY_ARGS); //获取当前需要调用的 Fragment 的参数
            if (args != null) {
            ...
            FragmentTransaction trans = getSupportFragmentManager()
                    .beginTransaction();//FragmentTransaction
                                      // 对fragment进行添加,移除,替换,以及执行其他动作,详见 有道云
            trans.replace(R.id.container, fragment, TAG); //以代替的方式调用 fragment
